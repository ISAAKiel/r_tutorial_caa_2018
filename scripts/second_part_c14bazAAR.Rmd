...
The c14 age is not the correct absolute age of a date. To correct the age value we have to apply c14 calibration. c14bazAAR offers a method to individually calibrate all dates in a c14_date_list. The actual calibration is done by `Bchron::BchronCalibrate()`.

```{r}
dates_head <- as.c14_date_list(dates[1:100,])

calibrate(dates_head)
```
...
Almost all databases have information about the sample material, but they are not standardized. 

```{r}
calpal <- get_CalPal() %>% dplyr::sample_n(1000) %>% as.c14_date_list()
...
Selection by country is a valuable mean of filtering especially for those dates without coordinates. Unfortunately the country information in the source databases is not very reliable and the country names are not standardized across (and sometimes even within) databases.
...
```{r}
View(head(countrycode::codelist))

country_thesaurus

adrac %>% standardize_country_name() %>% dplyr::select(country, country_thes)
```
...
duplicate_selection$duplicate_group
...
c14bazAAR provides the function `remove_duplicates()`, but it removes a lot of valuable information.
...
bawue <- st_read(file.path(tempdir(), "AX_Gebiet_Bundesland.shp"))

plot(bawue$geometry)

southwestern_germany <- germany %>%
  dplyr::filter(lat < 50, lon < 11, lon > 7.5)

southwestern_germany_sf <- southwestern_germany %>%
  as.c14_date_list()%>%
  as.sf %>%
  st_transform(crs = 25832)
  
bawue_dates <- southwestern_germany_sf %>%
  st_intersection(y = bawue)

plot(bawue_dates$geom, add = TRUE)
